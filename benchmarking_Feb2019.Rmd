...
#Tom Nichols, below. This, however, makes computed z's disagree with Draper e.g.!
if (tombit==1){
...
# (Stdev implied from N & mymean)
  mymat<-as.data.frame(matrix(NA,nrow=nuni,ncol=ncontext))
    ...
    myNmat <-data.frame(matrix(pmax(50,rnorm(nuni*ncontext,500,200)),nrow=nuni)) # just set N as random for each cell
    myNmat <- round(myNmat)
for (u in 1:nuni){
   mymean<-.5+runif(1)/2
    mymat[u,] <- rbinom(ncontext,rowSums(myNmat)[u],mymean)/rowSums(myNmat)[u]
}
