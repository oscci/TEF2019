...
Needless, to say, this is all quite technical, but even if you don't follow the maths, I suggest you just consider the analyses reported below, in which I compare the benchmarking output from the Draper and Gittoes method (henceforth DG) with that from my alternative method (henceforth DB).
...
The method used to do benchmarking comes from [Draper and Gittoes (2004)] (https://rss.onlinelibrary.wiley.com/doi/abs/10.1111/j.1467-985X.2004.apm12.x), and is explained in this [talk by Draper, 2006.](https://www.hesa.ac.uk/data-and-analysis/performance-indicators/benchmarks). A more comprehensive statistical treatment and critique can be found [here](https://users.soe.ucsc.edu/~draper/draper-talk-for-siena-meeting-12sep06.pdf).

A worked example of how benchmarks are derived in TEF can be found in Annex B of [this document] (https://www.officeforstudents.org.uk/media/c32f61aa-ceb7-4bbb-9f87-2cfc60f2bf89/tefyearfour_metricstechnicalalgorithmsandrebuildinstructions_revised20181219.pdf). This follows the same method as used in the examples by Draper (2006). Essentially, the benchmark for an institution is an average of indicator values weighted by the numbers with each combination of contextual factors in the institution. While cumbersome, it is not difficult to compute. The difference between obtained values and benchmarked value can then be computed, to see if the institution is scoring above expectation (positive difference) or below expectation (negative difference).
...
smallworldoutcomes  %>% kable(booktabs = T, caption = "Small World Example, % passing") %>% 
  kable_styling()
...
```{r AnnexB.eg}
# This is set out differently from Draper and Gittoes, with data just on one provider
...
[!Figure 1: Algebra for computing standard error of difference score, from Draper (2006)](Draper_Gittoes algebra.png)

Note that in formula 7, the lambda term is squared.

A function to compute the benchmarks, difference and standard errors is given below, and its use illustrated with the 'small world' dataset, showing that results agree with those of Draper (2006).
...
vabslam<-vnolambda<-vDB<-vDG #make vector for variance estimate with DB modified formula, or no lambda
...
for (k in 1:nrow(Ns)){
  for (i in 1:nrow(Ns)){
    for (j in 1:ncol(Ns)){
      ...
      addbit <-lambda*outcomes[k,j]
      addbit1 <-lambda^2*localvar[k,j] #this is the original Draper formula
      addbit2<-outcomes[k,j] #no lambda
      addbit3<-mymult*sqrt(abs(lambda))*localvar[k,j] #DB; draper formula with mymult*sqrt rather than squared lambda
      addbit4 <-abs(lambda)*localvar[k,j]
        
        if(is.na(addbit1)==FALSE){
        d[i] <- d[i]+addbit #this should recreate the previously computed difference values in diff
        vDG[i]<-vDG[i]+addbit1 #original Draper Gittoes
        vnolambda[i] <- vnolambda[i]+addbit2
        vDB[i] <- vDB[i]+addbit3
        vabslam[i]<-vabslam[i]+addbit4
...
allz<-data.frame(cbind(allbench,zDG,zDB,zabslam))
colnames(allz) <- c('N','observed','benchmark','diff','z.DG','z.DB','z.abslambda')
return(allz)
}
```

```{r smallworldSE}
# apply to Draper's toy data

mymult<-.5 #multiplier for lambda term in DB version
...
As noted by Draper (2006) the absolute z-scores given by this method seem remarkably large: around 80% of universities have absolute z-scores larger than 2. In this toy example, we see z-scores with absolute values of 5, 9 and 20. 
...
for(mymean in c(.6)){#mean benchmark level
  rerun<-0 #set to 1 to simulate data (simulation takes a few minutes)
...
#Plot the z-scores
hist(simz[,5],breaks=40,main='Draper-Gittoes z-scores from random data',xlab='z-score')
...
text(80,100,mytext)
plot(log(simz[,1]),simz[,5],ylab='DGz',xlab='logN',main='Draper-Gittoes method with random data, z-score by log N')
...
This result is at odds with this statement from OfS

5.63 **As a test of the likelihood that a difference between a provider’s benchmark and its indicator is due to chance alone, a z-score +/- 3.0 means the likelihood of the difference being due to chance alone has reduced substantially and is negligible. As such, z-score values larger than +/- 3 provide very little additional statistical evidence than a z-score of +/-3.0 and should, for practical purposes, be considered as equivalent to +/-3.**
...
A great deal hinges on the computation of lambda, which, as is evident from Figure 1, is not simple. However, there are two reasons to have confidence in the computations done here. First, as specified in Formula (5) in Figure 1, when correctly computed, lambda can be used to compute the difference scores (observed minus expected values) for a metric. This is done in the **makeSE** function above, and it gives the correct results for difference scores. Second, the high numbers of extreme z-scores obtained with the **makeSE** function are not dissimilar to those obtained with real data. To demonstrate that point, we now go on to load in data from TEF year 2. We use this dataset (from 2017), as it has the awards (Bronze, Silver, Gold) as well as the raw data. The computed z-scores are provided in the raw data file, and are plotted for Core data for each metric below. It is evident that absolute z-scores greater than 3 are common - with the exact number varying from metric to metric, but ranging from 33% to 58%.
...
It is important to bear in mind that when working with simulated data, the precise details will depend on factors such as the variance and distribution of scores. I have not explored these aspects in detail. My purpose here is just to note that the distribution of z-scores from the Draper-Gittoes method is very strange, and it is plausible that there could be an algebraic error involved in the use of lambda squared, which leads to very small standard errors. I'm not clear whether the use of a square root lambda weighting makes sense statistically: I just tried this as an empirical alternative, and was interested to see that it seemed to behave in a more orderly manner.

```{r plotDGvsDB}

plot(simz[,5],simz[,6],xlab='Draper-Gittoes (lambda squared)',ylab='sqrt lambda method')
abline(v=-2,lty=2)
abline(v=2,lty=2)
```
...
The Draper-Gittoes z-scores are flagged for significance as follows:
 one point (- or +) for an absolute z-score of 2, AND an absolute difference between observed and expected values of 2%.
 two points (-- or ++) for an absolute z-score of 3, AND an absolute difference between observed and expected values of 3%.
...
print('Quantile breaks in zscores: ')
print(quantbreaks)
